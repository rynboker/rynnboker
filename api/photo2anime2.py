...
def photo2anime2():
    ...
    try:
        ...
        return jsonify({
            "creator": "Astri", 
            "error": "The external API request timed out. Please try again later.", 
            "status": 504
        })
    except requests.exceptions.RequestException as e:
        return jsonify({
            "creator": "Astri", 
            "error": f"Error with the external API request: {e}", 
            "status": 500
        })
    except ValueError:
        return jsonify({
            "creator": "Astri", 
            "error": "Received invalid JSON from the external API.", 
            "status": 500
        })
...
def serve_image(filename):
    try:
        ...
        return jsonify({
            "error": f"500 - Something went wrong: {e}", 
            "status": 500
        })
